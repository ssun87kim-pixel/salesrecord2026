...
if page == "대시보드":
    ...
    tgt     = float(t_s.sum())                                                           # 연간 목표
    ytd_tgt = sum(float(t_s[m])   for m in range(1, last_act_m + 1)) if last_act_m else 0  # YTD 목표
    act     = sum(float(act_s[m]) for m in range(1, last_act_m + 1)) if last_act_m else 0  # YTD 실적
    prev    = sum(float(py_s[m])  for m in range(1, last_act_m + 1)) if last_act_m else 0  # YoY 실적

    ar = act / ytd_tgt            if ytd_tgt else None   # 달성률 = YTD실적 / YTD목표
    gr = (act - prev) / abs(prev) if prev    else None   # YoY 성장률
    ...
    tgt_sum     = sum(tgt_row)                                       # 연간 목표 전체
    act_sum_ytd = sum(act_row[:last_act_m]) if last_act_m else 0    # YTD 실적
    py_sum_all  = sum(py_row)                                        # 전년 연간 전체
    py_sum_ytd  = sum(py_row[:last_act_m])  if last_act_m else 0    # 전년 동기 (YoY 기준)
    tbl_data["총합"] = [
        fmt_won(tgt_sum),
        fmt_won(act_sum_ytd),
        fmt_won(py_sum_all),
    ...
    def _style_q(df: pd.DataFrame) -> pd.DataFrame:
        """달성률 행: 초록/노랑/빨강, YoY 행: 초록/빨강 텍스트"""
        styles = pd.DataFrame("", index=df.index, columns=df.columns)
        for idx in df.index:
    ...
    for label, ch in tbl_chs:
        ...
        ch_ar       = ch_a_ytd / ch_t_annual  if ch_t_annual else None   # YTD실적 / 연간목표
        ch_ar_ytd   = ch_a_ytd / ch_ytd_tgt   if ch_ytd_tgt  else None   # YTD실적 / YTD목표
    ...
    summary_rows = [{
        ...
        f"기간 달성률 ({period_lbl})": fmt_pct(s["ar_ytd"]),   # YTD실적 / YTD목표
        "연간 달성률":                 fmt_pct(s["ar"]),        # YTD실적 / 연간목표
...
<div style="text-align:center;font-size:13px;color:#969696;margin-top:48px;padding:16px 0;border-top:1px solid #EBEBEB;">
  개발 및 수정문의: DESKER 김선영 &nbsp;|&nbsp; v1.0.0 &nbsp;|&nbsp; 2026-04-20 KST
</div>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    tab_tgt, tab_hist = st.tabs(["연간 목표 설정", "과거 실적 입력 (수정 불가)"])
